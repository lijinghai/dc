...
import { join, normalize, dirname, resolve, relative, basename } from 'pathe';
...
import fsp, { mkdir, writeFile } from 'fs/promises';
import { promisify } from 'util';
import { readdirSync, statSync, existsSync, promises } from 'fs';
  ...
  return {
    hooks: {
      "build:prepare"(ctx) {
        ...
        consola.info("Automatically detected entries:", chalk.cyan(ctx.options.entries.map((e) => chalk.bold(e.input.replace(ctx.options.rootDir + "/", "").replace(/\/$/, "/*"))).join(", ")), chalk.gray(["esm", res.cjs && "cjs", res.dts && "dts"].filter(Boolean).map((tag) => `[${tag}]`).join(" ")));
      }
    }
  };
});
...
function extractExportFilenames(exports, conditions = []) {
  ...
  return Object.entries(exports).flatMap(([condition, exports2]) => typeof exports2 === "string" ? { file: exports2, type: inferExportType(condition, conditions, exports2) } : extractExportFilenames(exports2, [...conditions, condition]));
}
...
function validateDependencies(ctx) {
  ...
  const implicitDependnecies = /* @__PURE__ */ new Set();
  ...
  for (const id of usedDependencies) {
    if (!ctx.options.externals.includes(id) && !id.startsWith("chunks/") && !ctx.options.dependencies.includes(getpkg(id)) && !ctx.options.peerDependencies.includes(getpkg(id))) {
...
function validatePackage(pkg, rootDir) {
  if (!pkg) {
    return;
  }
...
export { default } from '${entry.input}';`);
  ...
  for (const outputOptions of allOutputOptions) {
    ...
    for (const entry of output.filter((e) => e.type === "chunk")) {
  ...
  if (ctx.options.declaration) {
    ...
    const shebangPlugin2 = rollupOptions.plugins.find((p) => p && p.name === "unbuild-shebang");
...
function getRollupOptions(ctx) {
  ...
  return {
    ...
    input: Object.fromEntries(ctx.options.entries.filter((entry) => entry.builder === "rollup").map((entry) => [entry.name, resolve(ctx.options.rootDir, entry.input)])),
    output: [
...
async function typesBuild(ctx) {
  const entries = ctx.options.entries.filter((entry) => entry.builder === "untyped");
...
async function build(rootDir, stub, inputConfig = {}) {
  ...
  const options = defu(buildConfig, pkg.unbuild || pkg.build, inputConfig, preset, {
    rootDir,
    entries: [],
    clean: true,
    declaration: false,
